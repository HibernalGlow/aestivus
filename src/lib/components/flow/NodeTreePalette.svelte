<script lang="ts">
  ...
  function getDefaultTreeData(): TreeFolder[] {
    ...
    // 找出未分类的工具节点，添加到工具根目录
    const uncategorizedTools = NODE_DEFINITIONS.filter(
      (n) => n.category === "tool" && !categorizedTools.has(n.type)
    )
        ...
        children: [
          {
            ...
            icon: "Folder",
            expanded: false,
            items: toolCategories["tool-file"]
              .map((t) => buildNodeItem(t)!)
              .filter(Boolean),
            children: [],
          },
          {
            ...
            icon: "Package",
            expanded: false,
            items: toolCategories["tool-archive"]
              .map((t) => buildNodeItem(t)!)
              .filter(Boolean),
            children: [],
          },
          {
            ...
            icon: "Video",
            expanded: false,
            items: toolCategories["tool-media"]
              .map((t) => buildNodeItem(t)!)
              .filter(Boolean),
            children: [],
          },
          {
            ...
            icon: "Terminal",
            expanded: false,
            items: toolCategories["tool-system"]
              .map((t) => buildNodeItem(t)!)
              .filter(Boolean),
            children: [],
          },
          {
            ...
            icon: "FileText",
            expanded: false,
            items: toolCategories["tool-text"]
              .map((t) => buildNodeItem(t)!)
              .filter(Boolean),
            children: [],
          },
  ...
  function loadTreeData(): TreeFolder[] {
    ...
    if (saved) {
      try {
        ...
        // 检测新增的工具节点，添加到工具根目录
        const newToolNodes = NODE_DEFINITIONS.filter(
          (n) => n.category === "tool" && !existingNodeIds.has(n.type)
        )
          .map((n) => buildNodeItem(n.type)!)
          .filter(Boolean);
  ...
  function folderHasMatches(folder: TreeFolder, query: string): boolean {
    if (!query) return true;
    return (
      folder.items.some((item) => nodeMatches(item, query)) ||
      folder.children.some((c) => folderHasMatches(c, query))
    );
  }
...
<div class="h-full flex flex-col">
  <div class="p-3 border-b flex items-center gap-2">
    ...
    <button
      ...
      onclick={resetLayout}
      title="重置布局"><Trash2 class="w-4 h-4" /></button
    >
    <button
      class="p-1.5 rounded hover:bg-muted transition-colors"
      onclick={exportJson}
      title="导出 JSON"><Download class="w-4 h-4" /></button
    >
    <button
      class="p-1.5 rounded hover:bg-muted transition-colors"
      onclick={() => fileInput.click()}
      title="导入 JSON"><Upload class="w-4 h-4" /></button
    >
    <input
      bind:this={fileInput}
      type="file"
  ...
  <div class="flex-1 overflow-y-auto p-3 space-y-3">
    {#each treeData as folder (folder.id)}
      {#if folderHasMatches(folder, searchQuery)}
        ...
        {@const FolderIcon = (icons as any)[folder.icon] || Folder}
        <div>
          <button
          ...
          >
            {#if folder.expanded}<ChevronDown
                class="w-3 h-3"
              />{:else}<ChevronRight class="w-3 h-3" />{/if}
            <FolderIcon class="w-3.5 h-3.5 text-{color}-500" />
            <span>{folder.name}</span>
            {#if folder.items.length > 0}<span class="text-[10px] opacity-50"
                >({folder.items.length})</span
              >{/if}
          </button>
          ...
          {#if folder.expanded}
            {#if folder.items.length > 0}
              ...
              >
                {#each folder.items.filter( (item) => nodeMatches(item, searchQuery) ) as item (item.id)}
                  {@const Icon = (icons as any)[item.icon] || Terminal}
                  {@const isOver =
                    dragState.overItemId === item.id &&
                  ...
                  <div class="flex items-center gap-1 group">
                    <div
                      ...
                      onkeydown={(e) =>
                        e.key === "Enter" && addNode(item.type, item.label)}
                      role="button"
                      tabindex="0"
                    >
            ...
            {#each folder.children as subFolder (subFolder.id)}
              {#if folderHasMatches(subFolder, searchQuery)}
                {@const SubIcon = (icons as any)[subFolder.icon] || Folder}
                ...
                <div class="mt-2 ml-2">
                  ...
                  >
                    {#if subFolder.expanded}<ChevronDown
                        class="w-3 h-3"
                      />{:else}<ChevronRight class="w-3 h-3" />{/if}
                    <SubIcon class="w-3.5 h-3.5 text-{subColor}-500" />
                    <span>{subFolder.name}</span>
                    <span class="text-[10px] opacity-50"
                      >({subFolder.items.length})</span
                    >
                  </button>
                  ...
                  {#if subFolder.expanded}
                    ...
                    >
                      {#each subFolder.items.filter( (item) => nodeMatches(item, searchQuery) ) as item (item.id)}
                        ...
                        {@const isOver =
                          dragState.overItemId === item.id &&
                          dragState.overFolderId === subFolder.id}
                        <div class="flex items-center gap-1 group">
                          <div
                            class="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border transition-colors cursor-grab active:cursor-grabbing bg-card
                              {isOver
                            ...
                            draggable="true"
                            ondragstart={(e) =>
                              handleDragStart(e, subFolder.id, item)}
                            ondragover={(e) =>
                              handleDragOver(e, subFolder.id, item.id)}
                            ondragleave={(e) => handleDragLeave(e, item.id)}
                          ...
                          >
                            ...
                            />
                            <Icon
                              class="w-4 h-4 text-{subColor}-600 dark:text-{subColor}-400"
                            />
                            <span class="text-sm text-left flex-1"
                              >{item.label}</span
                            >
                          </div>
                          <button
                            class="p-1.5 rounded opacity-0 group-hover:opacity-100 hover:bg-muted transition-all"
                            onclick={() =>
                              openFullscreen(item.type, item.label)}
                            title="全屏打开"
                          >
                            <Maximize2
