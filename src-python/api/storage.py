...
# ========== PyO3 Bridge Functions ==========
# These functions are called directly by Rust via PyO3, bypassing FastAPI.
# They handle their own database sessions.
...
def rpc_list_layouts(args: dict) -> list:
    """列出所有布局 (PyO3 入口)"""
    with _get_session_ctx() as session:
        return list_layouts(session)


def rpc_get_layout(args: dict) -> dict | None:
    """获取布局 (PyO3 入口)"""
    node_type = args.get("node_type")
    with _get_session_ctx() as session:
        return get_layout(node_type, session)


def rpc_set_layout(args: dict) -> dict:
    """设置布局 (PyO3 入口)"""
    node_type = args.get("node_type")
    config = args.get("config", {})
    with _get_session_ctx() as session:
        request = LayoutConfigRequest(config=config)
        return set_layout(node_type, request, session)


def rpc_delete_layout(args: dict) -> dict:
    """删除布局 (PyO3 入口)"""
    node_type = args.get("node_type")
    with _get_session_ctx() as session:
        return delete_layout(node_type, session)


def rpc_list_presets(args: dict) -> list:
    """列出预设 (PyO3 入口)"""
    node_type = args.get("node_type")
    with _get_session_ctx() as session:
        return list_presets(node_type, session)


def rpc_get_defaults(args: dict) -> dict | None:
    """获取默认预设 (PyO3 入口)"""
    node_type = args.get("node_type")
    with _get_session_ctx() as session:
        return get_defaults(node_type, session)


def rpc_set_defaults(args: dict) -> dict:
    """设置默认预设 (PyO3 入口)"""
    node_type = args.get("node_type")
    fullscreen = args.get("fullscreen_preset_id")
    normal = args.get("normal_preset_id")
    with _get_session_ctx() as session:
        request = DefaultPresetRequest(
            fullscreen_preset_id=fullscreen,
            normal_preset_id=normal
        )
        return set_defaults(node_type, request, session)
