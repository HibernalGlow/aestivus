...
def rpc_execute_node(args: dict) -> dict:
    """执行单个节点 (PyO3 入口)"""
    import asyncio
    ...
    # 同步执行异步函数
    result = asyncio.run(execute_node(request))
    
    # 转换为可序列化的字典
    return result.model_dump()


def rpc_execute_flow(args: dict) -> dict:
    """执行流程 (PyO3 入口)"""
    import asyncio
    
    # 构建请求对象
    nodes = [FlowNode(**n) for n in args.get("nodes", [])]
    edges = [FlowEdge(**e) for e in args.get("edges", [])]
    ...
    # 同步执行异步函数
    result = asyncio.run(execute_flow(request))
    
    # 转换为可序列化的字典
    return result.model_dump()


def rpc_get_node_types(args: dict) -> list:
    """获取节点类型列表 (PyO3 入口)"""
