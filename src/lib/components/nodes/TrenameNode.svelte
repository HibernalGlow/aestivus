<script lang="ts">
  /**
   * TrenameNode - 可调节大小的批量重命名节点
   * 
   * 尺寸模式：
   * - compact: 紧凑模式，只显示核心操作
   * - medium: 中等模式，显示文件树预览
   * - expanded: 展开模式，显示完整表格编辑
   */
  ...
  import { 
    ...
    File, Folder, Trash2, Maximize2, Minimize2, GripVertical
  } from '@lucide/svelte';
  
  export let id: string;
  export let data: { config?: { path?: string }; logs?: string[]; expanded?: boolean } = {};
  ...
  // 尺寸模式
  type SizeMode = 'compact' | 'medium' | 'expanded';
  let sizeMode: SizeMode = data?.expanded ? 'expanded' : 'compact';
  ...
  $: nodeWidth = sizeMode === 'compact' ? 320 : sizeMode === 'medium' ? 420 : 560;
  $: nodeHeight = sizeMode === 'compact' ? 'auto' : sizeMode === 'medium' ? 400 : 600;

  function log(msg: string) { logs = [...logs.slice(-20), msg]; }
  ...
  function cycleSizeMode() {
    sizeMode = sizeMode === 'compact' ? 'medium' : sizeMode === 'medium' ? 'expanded' : 'compact';
  }
...
{#snippet treeNode(node: TreeNode, path: string, depth: number)}
  ...
  <div class="flex items-center gap-1 py-0.5 hover:bg-muted/50 rounded text-xs" style="padding-left: {depth * 12}px">
    ...
    {:else}
      <span class="w-4"></span>
      <File class="w-3 h-3 text-blue-500" />
    {/if}
    <span class="truncate flex-1" title={name}>{name}</span>
    {#if tgt && tgt !== name}
      <span class="text-muted-foreground">→</span>
      <span class="truncate text-green-600 max-w-20" title={tgt}>{tgt}</span>
    {/if}
    <span class="w-2 h-2 rounded-full {status === 'ready' ? 'bg-green-500' : status === 'pending' ? 'bg-yellow-500' : 'bg-gray-300'}"></span>
  </div>
...
<div 
  class="rounded-lg border-2 bg-card p-3 transition-all {phase === 'error' ? 'border-red-500' : phase === 'completed' ? 'border-green-500' : phase === 'scanning' || phase === 'renaming' ? 'border-blue-500 shadow-lg' : 'border-border'}"
  style="width: {nodeWidth}px; {sizeMode !== 'compact' ? `height: ${nodeHeight}px` : ''}"
>
  ...
  <div class="flex items-center justify-between mb-2">
    ...
    <div class="flex items-center gap-1">
      <Button variant="ghost" size="icon" class="h-6 w-6" onclick={cycleSizeMode} title="切换大小">
        {#if sizeMode === 'compact'}<Maximize2 class="h-3 w-3" />{:else}<Minimize2 class="h-3 w-3" />{/if}
      </Button>
  ...
  <div class="space-y-2">
    ...
    <div class="flex gap-1">
      ...
      <Button variant="ghost" size="icon" class="h-7 w-7" onclick={selectFolder} disabled={isRunning}><FolderOpen class="h-3 w-3" /></Button>
      <Button variant="ghost" size="icon" class="h-7 w-7" onclick={pastePath} disabled={isRunning}><Clipboard class="h-3 w-3" /></Button>
    </div>
    ...
    <div class="flex gap-1">
      ...
      <Button variant="ghost" size="sm" class="h-7 w-7 p-0" onclick={handleUndo} disabled={!lastOperationId} title="撤销">
        <Undo2 class="h-3 w-3" />
      </Button>
      <Button variant="ghost" size="sm" class="h-7 w-7 p-0" onclick={clear} title="清空">
    ...
    {#if sizeMode !== 'compact'}
      ...
      <div class="flex gap-2 text-xs">
        <label class="flex items-center gap-1 flex-1">
          ...
          <Input bind:value={excludeExts} class="h-6 text-xs flex-1" placeholder=".json,.txt" />
        </label>
        <label class="flex items-center gap-1">
    ...
    <!-- 文件树预览（medium/expanded 模式） -->
    {#if sizeMode !== 'compact' && treeData.length > 0}
      <div class="border rounded flex-1 overflow-hidden flex flex-col min-h-0">
        <div class="text-xs font-medium p-1 border-b bg-muted/30 flex items-center justify-between">
          <span>文件树</span>
    ...
    <!-- 日志区域 -->
    {#if logs.length > 0}
      <div class="border rounded bg-muted/20 {sizeMode === 'compact' ? 'max-h-16' : 'max-h-24'} overflow-y-auto">
        <div class="flex items-center justify-between px-1 py-0.5 border-b bg-muted/30">
        ...
        <div class="p-1 space-y-0.5">
          {#each logs as log}
            <div class="text-xs font-mono text-muted-foreground truncate" title={log}>{log}</div>
          {/each}
        </div>
      </div>
    {/if}
